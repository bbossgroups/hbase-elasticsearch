 ...
 * @version 1.0
 */
public class ES2HBaseDemo {
	private static Logger logger = LoggerFactory.getLogger(ES2HBaseDemo.class);
	public static void main(String args[]){
		ES2HBaseDemo dbdemo = new ES2HBaseDemo();
	...
	public void scheduleTimestampImportData(boolean dropIndice){
		...
		hBaseOutputConfig.setFamiliy("info").setHbaseTable("esdemo") ;//指定需要同步数据的hbase表名称;
		hBaseOutputConfig.setRowKeyField("docId").addHbaseClientProperty("hbase.zookeeper.quorum","192.168.137.133")  //hbase客户端连接参数设置，参数含义参考hbase官方客户端文档
...
//					 .setScheduleDate(date) //指定任务开始执行时间：日期
		...
		//增量配置开始
		importBuilder.setLastValueColumn("logId");//手动指定数字增量查询字段
		importBuilder.setFromFirst(false);//任务重启时，重新开始采集数据，true 重新开始，false不重新开始，适合于每次全量导入数据的情况，如果是全量导入，可以先删除原来的索引数据
		importBuilder.setLastValueStorePath("es2hbase_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
...
//				String id = context.getStringValue("_id");
				...
				context.addFieldValue("docId",docId);//添加文档id，将作为HBase rowKey
...
//					context.setDrop(true);
		...
		/**
		 * 构建DataStream，执行es数据到mongodb的同步操作
		 */
