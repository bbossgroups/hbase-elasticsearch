 ...
 * @version 1.0
 */
public class DB2HbaseWithFamilyColumnDemo {
	private static Logger logger = LoggerFactory.getLogger(DB2HbaseWithFamilyColumnDemo.class);
	public static void main(String args[]){
		DB2HbaseWithFamilyColumnDemo dbdemo = new DB2HbaseWithFamilyColumnDemo();
...
//					 .setScheduleDate(date) //指定任务开始执行时间：日期
		...
		importBuilder.setLastValueStorePath("db2hbase_customimport");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
